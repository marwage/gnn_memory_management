...
static void BM_Layer_Dropout_Chunked_Flickr_Forward(benchmark::State &state) {
    ...
    for (auto _ : state) {
        activations = dropout.forward(&features_chunked);
    }
...
static void BM_Layer_Dropout_Flickr_Chunked_Backward(benchmark::State &state) {
    ...
    std::vector<Matrix<float>> *activations = dropout.forward(&features_chunked);
    ...
    for (auto _ : state) {
        gradients = dropout.backward(&in_gradients_chunked);
    }
