...
BENCHMARK(BM_Layer_Add_Flickr_Pipelined_Forward)->RangeMultiplier(2)->Range(1 << 16, 1 << 16);

static void BM_Layer_Add_Reddit_Pipelined_Forward(benchmark::State &state) {
    AddPipelined add;
    benchmark_add_chunked(&add, reddit, state, true);
}
BENCHMARK(BM_Layer_Add_Reddit_Pipelined_Forward)->RangeMultiplier(2)->Range(1 << 16, 1 << 17);

static void BM_Layer_Add_Products_Pipelined_Forward(benchmark::State &state) {
    AddPipelined add;
    benchmark_add_chunked(&add, products, state, true);
}
BENCHMARK(BM_Layer_Add_Products_Pipelined_Forward)->RangeMultiplier(2)->Range(1 << 16, 1 << 21);

static void BM_Layer_Add_Ivy_Pipelined_Forward(benchmark::State &state) {
    AddPipelined add;
    benchmark_add_chunked(&add, ivy, state, true);
}
BENCHMARK(BM_Layer_Add_Ivy_Pipelined_Forward)->RangeMultiplier(2)->Range(1 << 16, 1 << 19);
