...
int test_relu_pipeline(long chunk_size) {
    std::string path;
...
TEST_CASE("ReLU, chunked, pipeline", "[relu][chunked][pipeline]") {
    CHECK(test_relu_pipeline(1 << 12));
    CHECK(test_relu_pipeline(1 << 8));
    CHECK(test_relu_pipeline(1 << 6));
}
